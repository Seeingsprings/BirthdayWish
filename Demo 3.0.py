...
# 心形脉动参数
PULSE_ENABLED = True
PULSE_AMPLITUDE = 0.06  # 建议 0.03 ~ 0.08 之间，越大越明显
PULSE_SPEED_HZ = 0.8    # 每秒脉动频率
_PULSE_TRACK = []       # 追踪活动弹窗：{root, base_x, base_y, w, h}
...
def show_warn_tip(x, y, w, h):
    ...
    try:
        ...
        def _on_destroy(_e=None):
            # 清理追踪，避免残留
            for i in range(len(_PULSE_TRACK) - 1, -1, -1):
                if _PULSE_TRACK[i]["root"] is root:
                    _PULSE_TRACK.pop(i)
                    break
...
def main():
    ...
    # 心形脉动动画循环：统一缩放所有活动弹窗位置
    if PULSE_ENABLED:
        start_ms = int(time.time() * 1000)
        def pulse_loop():
            now_ms = int(time.time() * 1000)
            t = (now_ms - start_ms) / 1000.0
            scale = 1.0 + PULSE_AMPLITUDE * math.sin(2 * math.pi * PULSE_SPEED_HZ * t)
            cx = SCREEN_W // 2
            cy = SCREEN_H // 2
            # 更新所有活动窗口位置
            for item in list(_PULSE_TRACK):
                root = item.get("root")
                if not (root and str(root)):
                    continue
                bx = item["base_x"]
                by = item["base_y"]
                w = item["w"]
                h = item["h"]
                nx = int(cx + (bx - cx) * scale)
                ny = int(cy + (by - cy) * scale)
                # 边界保护
                nx = max(w // 2, min(nx, SCREEN_W - w // 2))
                ny = max(h // 2, min(ny, SCREEN_H - h // 2))
                try:
                    root.geometry(f"{w}x{h}+{nx - w // 2}+{ny - h // 2}")
                except Exception:
                    pass
            master.after(33, pulse_loop)  # 约 30 FPS
        master.after(0, pulse_loop)
